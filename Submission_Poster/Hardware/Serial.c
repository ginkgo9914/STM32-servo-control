...
uint8_t Serial_TxPacket[2];				//定义发送数据包数组，数据包格式：FF 01 02 03 04 FE
uint8_t Serial_RxPacket[4];				//定义接收数据包数组
uint8_t Serial_RxFlag;					//定义接收数据包标志位
static uint32_t SendFlag = true;	//发送数据定时器，用于控制发送数据的频率
...
void Serial_Init(void)
{
	...
	/*初始化发送数据包为中性值（90表示角度居中），避免心跳包发送随机数据*/
	Serial_TxPacket[0] = 90;
	Serial_TxPacket[1] = 90;
}
