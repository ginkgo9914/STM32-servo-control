#include "Timer.h"
#include "stddef.h"
typedef void (*timer_init_func)(void);
typedef void (*timer_stop_func)(void);
...
const timer_init_func timer_init[3] = {NULL, Timer1_Init, Timer2_Init};
const timer_stop_func timer_stop[3] = {NULL, Timer1_Stop, Timer2_Stop};
/**
  * @brief  定时器初始化
  * @param  type：定时器类型 目前支持 1, 2
  * @retval 无
  */
void Timer_Init(int8_t type){
	if (timer_init[type] != NULL){
		timer_init[type]();
	}
}

/**
 * @brief  定时器1初始化 MPU6050中断计算。
 */
static void Timer1_Init(void){
	/*开启时钟*/
...
/**
  ...
  * @param  type：定时器类型 目前支持 1, 2
  */
void Timer_Stop(int8_t type){
	if (timer_stop[type] != NULL){
		timer_stop[type]();
	}
}
